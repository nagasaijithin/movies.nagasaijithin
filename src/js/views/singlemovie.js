...
function singleMovieRender(data, movie_id) {
        ...
        <div class="sm-bio">
            <div class="bio-main">
                ...
                <div class="rating">
                    <h3><span>${
                      data.vote_average
                    }</span> / 10 <span><i class="fas fa-star"></i></span></h3>
                </div>
                <div class="overviwe">
                    <h3>Overview</h3>
                    <p>${data.overview}</p>
                ...
                <div class="button cats-btn">
                    <a href="${
                      data.homepage
                    }" class="btn" target="_black">Websit</a>
                    <a href="${imdblink}${
    data.imdb_id
  }" class="btn" target="_black">IMDB</a>
                    <p href="#" data-video_id="${
                      data.id
                    }" class="btn video-btn">Trailer</p>
                </div>
            </div>
  ...
  root
    .querySelector(".all-cats .slidem")
    .addEventListener("click", function () {
      if (imgsliderdeg < 0) {
        root.querySelector(
          ".ani-wapper .persons"
        ).style.transform = `translate3d(-${imgsliderdeg}px, 0, 0)`;
      } else {
        root.querySelector(
          ".ani-wapper .persons"
        ).style.transform = `translate3d(-${(imgsliderdeg -= 80)}px, 0, 0)`;
      }
    });

  root
    .querySelector(".all-cats .slidep")
    .addEventListener("click", function () {
      ...
      if (80 * howManypersons - wapperWidth < imgsliderdeg) {
        ...
        root.querySelector(
          ".ani-wapper .persons"
        ).style.transform = `translate3d(-${val}px, 0, 0)`;
      } else {
        let val;
        val = imgsliderdeg += 80;
        root.querySelector(
...
function recommendeddata(data) {
  if (data.results.length > 5) {
    ...
    for (let i = 0; i < data.results.length; i++) {
      recommend += `
                   <div class="sm-items item${
                     i + 1
                   } movieclick" data-movieid="${data.results[i].id}">
                           <img src='${
    ...
    root.querySelector(".sm-grid").innerHTML = `<div class="go-back-wapper">
    <a href="/" class="go-back-wapper-btn">Go Home</a>
    </div>`;
  }
  root
    .querySelector(".video-wapper .nav__handel-bar")
...
function personRender(data) {
  ...
  root
    .querySelector(".person-wapper .nav__handel-bar")
    .addEventListener("click", function () {
